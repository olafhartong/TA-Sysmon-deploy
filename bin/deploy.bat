...
sc query "Sysmon" | Find /c "RUNNING" 2>nul && echo %DATE%-%TIME% Sysmon found, checking version && c:\windows\sysmon.exe | Find /c "System Monitor v7.02" 1>nul && echo %DATE%-%TIME% Sysmon already up to date, exiting && exit

sc query "Sysmon" | Find /c "RUNNING" 1>nul && c:\windows\sysmon.exe | Find /c "System Monitor v7.02" >nul || echo %DATE%-%TIME% Sysmon binary is outdated, un-installing && c:\windows\sysmon -u
